...
use chimitheque_types::{name::Name, requestfilter::RequestFilter};
...
fn build_request(request_filter: &RequestFilter) -> ehttp::Request {
    ehttp::Request::get(format!(
        "https://localhost:8443/back/products/names{request_filter}",
    ))
...
fn get_names_from_response(response: &ehttp::Response) -> Result<Option<(Vec<Name>, u64)>, String> {
    match parse_retrieve_names_response(response) {
...
pub fn load_suggestions(
    ...
    ehttp::fetch(request, move |mayerr_response| match mayerr_response {
        Ok(response) => {
            ...
            // We don't need to log the errors here as they are already logged in `get_names_from_response`.
            if let Ok(maybe_names) = get_names_from_response(&response)
                && let Some(names) = maybe_names
            {
                let items: Vec<SelectItem> = names
                    ...
                    .collect();
                let total = match usize::try_from(names.1) {
                    Ok(total) => total,
                    Err(e) => {
                        log::error!("{e}");
                        return;
...
fn parse_retrieve_names_response(response: &ehttp::Response) -> Result<(Vec<Name>, u64), AppError> {
        ...
        200 => {
            if let Some(text_response) = response.text() {
                match serde_json::from_str(text_response) {
                    ...
                    Err(e) => {
                        log::error!("parse_retrieve_names_response: InternalError: {e}");
                        Err(AppError::InternalError(e.to_string()))
                    }
                }
            } else {
                log::error!("parse_retrieve_names_response: UnexpectedEmptyResponse");
                Err(AppError::UnexpectedEmptyResponse)
            }
        }
        _ => {
            if let Some(text_response) = response.text() {
                log::error!("parse_retrieve_names_response: NotOkHTTPResponse: {text_response}");
                Err(AppError::NotOkHTTPResponse(text_response.to_string()))
            } else {
                log::error!(
                    "parse_retrieve_names_response: NotOkHTTPResponse: {}",
                    response.status
                );
