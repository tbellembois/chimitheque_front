    ...
    ehttp::fetch(request, move |mayerr_response| match mayerr_response {
        Ok(response) => {
            ...
            match parse_response(&response) {
                Ok(response_precautionary_statements_and_count) => {
                    let items: Vec<SelectItem> = response_precautionary_statements_and_count
                        .0
                        .into_iter()
                        .map(|precautionary_statement| SelectItem {
                            id: precautionary_statement.precautionary_statement_id,
                            label: precautionary_statement.precautionary_statement_label,
                        })
                        .collect();
                    let total = match usize::try_from(response_precautionary_statements_and_count.1)
                    {
                        Ok(total) => total,
                        Err(e) => {
                            elog!(error, format!("{e}"));
