    ...
    ehttp::fetch(request, move |mayerr_response| match mayerr_response {
        Ok(response) => {
            ...
            match parse_response(&response) {
                Ok(response_symbolss_and_count) => {
                    let items: Vec<SelectItem> = response_symbolss_and_count
                        .0
                        .into_iter()
                        ...
                        .collect();
                    let total = match usize::try_from(response_symbolss_and_count.1) {
                        Ok(total) => total,
                        Err(e) => {
                            elog!(error, format!("{e}"));
                            return;
...
fn parse_response(response: &ehttp::Response) -> Result<(Vec<Symbol>, u64), AppError> {
