...
use chimitheque_types::{tag::Tag, requestfilter::RequestFilter};
...
fn build_request(request_filter: &RequestFilter) -> ehttp::Request {
    ehttp::Request::get(format!(
        "https://localhost:8443/back/products/tags{request_filter}",
    ))
...
fn get_tags_from_response(response: &ehttp::Response) -> Result<Option<(Vec<Tag>, u64)>, String> {
    match parse_retrieve_tags_response(response) {
...
pub fn load_suggestions(
    ...
    ehttp::fetch(request, move |mayerr_response| match mayerr_response {
        Ok(response) => {
            ...
            // We don't need to log the errors here as they are already logged in `get_tags_from_response`.
            if let Ok(maybe_tags) = get_tags_from_response(&response)
                && let Some(tags) = maybe_tags
            {
                let items: Vec<SelectItem> = tags
                    .0
                    .into_iter()
                    .map(|tag| SelectItem {
                        id: tag.tag_id,
...
fn parse_retrieve_tags_response(response: &ehttp::Response) -> Result<(Vec<Tag>, u64), AppError> {
        ...
        200 => {
            if let Some(text_response) = response.text() {
                match serde_json::from_str(text_response) {
                    ...
                    Err(e) => {
                        log::error!("parse_retrieve_tags_response: InternalError: {e}",);
                        Err(AppError::InternalError(e.to_string()))
                    }
                }
            } else {
                log::error!("parse_retrieve_tags_response: UnexpectedEmptyResponse");
                Err(AppError::UnexpectedEmptyResponse)
            }
        }
        _ => {
            if let Some(text_response) = response.text() {
                log::error!("parse_retrieve_tags_response: NotOkHTTPResponse: {text_response}",);
                Err(AppError::NotOkHTTPResponse(text_response.to_string()))
            } else {
                log::error!(
                    "parse_retrieve_tags_response: NotOkHTTPResponse: {}",
                    response.status
                );
