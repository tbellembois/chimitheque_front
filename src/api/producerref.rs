    ...
    ehttp::fetch(request, move |mayerr_response| match mayerr_response {
        Ok(response) => {
            ...
            match parse_response(&response) {
                Ok(response_producer_refs_and_count) => {
                    let items: Vec<SelectItem> = response_producer_refs_and_count
                        .0
                        .into_iter()
                        .map(|producer_ref| SelectItem {
                            id: producer_ref.producer_ref_id,
                            label: producer_ref.producer_ref_label,
                        })
                        .collect();
                    let total = match usize::try_from(response_producer_refs_and_count.1) {
                        Ok(total) => total,
                        Err(e) => {
                            elog!(error, format!("{e}"));
                            return;
...
fn parse_response(response: &ehttp::Response) -> Result<(Vec<ProducerRef>, u64), AppError> {
