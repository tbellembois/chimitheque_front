use crate::{
    elog, error::apperror::AppError, keycloak::get_token, types::SharedPersonAndCountList,
};
...
fn build_request(request_filter: &RequestFilter) -> ehttp::Request {
    ehttp::Request::get(format!(
        "https://localhost:8443/back/people{request_filter}",
    ))
...
pub fn get_people(
    ...
    shared_maybe_people_and_count: SharedPersonAndCountList,
    append: bool,
    ...
    elog!(info, format!("getting people (offset: {offset})"));

    ehttp::fetch(request, move |mayerr_response| {
        match mayerr_response {
            Ok(response) => {
                // Acquire lock on current people.
                let mut maybe_current_people_and_count = match shared_maybe_people_and_count.lock()
                {
                    Ok(locked) => locked,
                    Err(e) => {
                        elog!(error, format!("{e}"));
                ...
                match parse_response(&response) {
                    Ok(mut response_people_and_count) => {
                        if append {
                            if let Some(current_people_and_count) =
                                maybe_current_people_and_count.as_mut()
                            {
                                current_people_and_count
                                    .0
                                    .append(&mut response_people_and_count.0);
                            } else {
                                *maybe_current_people_and_count = Some(response_people_and_count);
                            }
                        } else {
                            *maybe_current_people_and_count = Some(response_people_and_count);
                        }
        ...
        elog!(
            info,
            format!(
                "getting people (offset: {offset}) {} done",
...
pub fn load_suggestions(
    ...
    ehttp::fetch(request, move |mayerr_response| match mayerr_response {
        Ok(response) => {
            ...
            match parse_response(&response) {
                Ok(response_people_and_count) => {
                    let items: Vec<SelectItem> = response_people_and_count
                        .0
                        .into_iter()
                        .map(|person| SelectItem {
                            id: person.person_id,
                            label: person.person_email,
                        })
                        .collect();
                    let total = match usize::try_from(response_people_and_count.1) {
                        Ok(total) => total,
                        Err(e) => {
                            elog!(error, format!("{e}"));
                            return;
...
fn parse_response(response: &ehttp::Response) -> Result<(Vec<Person>, u64), AppError> {
